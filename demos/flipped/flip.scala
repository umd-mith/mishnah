...
implicit def nodeListToScala(nodeList: NodeList) =
...
val byId = doc.getElementsByTagName("link").toList.filterNot(
...
mappings.foreach {
  case (file, mappings) =>
    ...
    val elements = (
      doc.getElementsByTagName("damageSpan").toList ++
      doc.getElementsByTagName("anchor").toList ++ 
      doc.getElementsByTagName("pb").toList ++
      doc.getElementsByTagName("milestone").toList
    ).groupBy(e =>
      Option(
