<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" 
   ...
   <xsl:param name="wits"
      select="'S07397,S07326,S00651,S00483,S01520,S08174,P00001,P179204,S07319,S08010,P00002,S07204,S07106,S07394'"/>
   ...
   <xsl:template match="tei:ab">
      <div class="apparatus">
         <xsl:for-each-group select="tei:app"
            group-adjacent="boolean(tei:rdgGrp[@n = 'empty']/tei:rdg[@wit = $base])">
               ...
               <xsl:otherwise>
                  <xsl:for-each select="current-group()">
                     <xsl:choose>
                        ...
                        <xsl:when
                           test="following-sibling::*[1]/tei:rdgGrp[@n = 'empty']/tei:rdg[contains(@wit, $base)]"
                           >
      ...
      <xsl:if test="count(tei:rdgGrp[1]/tei:rdg) &lt; count($witList) + 1">
         <div class="locusGrp" id="{@xml:id}">
            ...
            <span class="matchesBase">
               <span class="sigla">
                  <xsl:variable name="witsInGrp"
                     select="tei:rdgGrp[tei:rdg[contains(@wit, $base)]]/tei:rdg/@wit"/>
                  <xsl:value-of
                     select="
                        for $i in $witList
                        return
            ...
            <xsl:for-each
               select="tei:rdgGrp[not(@n = 'empty')][not(tei:rdg/@wit[contains(., $base)])]">
               <span class="rdgGrp grp-{@n}">
                  <span class="rdg">
                     <xsl:for-each select="tei:rdg[1]/tei:ptr/@target">
                        <xsl:variable name="id" select="tokenize(., ' ')"/>
                        <xsl:apply-templates
                           select="$w-data/id(substring-after(tokenize($id[1], '-')[1], '#'))">
                           <xsl:with-param name="h"
                              select="
                                 if (contains($id, '-h')) then
                                    tokenize($id[1], '-')[last()]
                                 else
                                    ''"
                              tunnel="yes"/>
            ...
            <xsl:if test="current-group()/tei:rdgGrp[@n = 'empty']">
               <span class="rdgGrp empty">
                  <span class="sigla">
                     <xsl:variable name="witsInGrp"
                        select="current-group()/tei:rdgGrp[@n = 'empty']/tei:rdg/@wit"/>
                     <xsl:value-of
                        select="
                           for $i in $witList
                           return
   ...
   <xsl:template name="baseAbsentAtLocus">
      ...
      <div class="locusGrp" id="{@xml:id}">
         <span class="rdg">
            <xsl:for-each
               select="current-group()[1]/preceding-sibling::*[1]/tei:rdgGrp/tei:rdg[@wit = $base]/tei:ptr/@target">
               <xsl:variable name="id" select="tokenize(., ' ')"/>
               <xsl:apply-templates
                  select="$w-data/id(substring-after(tokenize($id[1], '-')[1], '#'))">
                  <xsl:with-param name="h"
         ...
         <xsl:variable name="groups">
            <xsl:for-each-group
               select="(current-group()[1]/preceding-sibling::*[1] | tei:rdg | current-group())//tei:rdg"
               group-by="@wit">
               ...
               <grp n="{@wit}">
                  <sort>
                     <xsl:value-of
                        select="
                           for $t in current-group()/tei:ptr/@target
                           return
                              $w-data/id(replace(substring-after($t, '#'), '-h[\d]', ''))"
                     />
                  </sort>
                  <use>
         ...
         <xsl:if
            test="$groups/xhtml:grp[@n = $base]/xhtml:sort = $groups/xhtml:grp[@n != $base]/xhtml:sort">
            <span class="matchesBase merged">
               <span class="sigla">
                  <!-- too long an expression? -->
                  <xsl:variable name="witsInGrp"
                     select="$groups/xhtml:grp[@n != $base][xhtml:sort = $groups/xhtml:grp[@n = $base]/xhtml:sort]/@n"/>
                  <xsl:value-of
                     select="
                        for $i in $witList
                        return
         ...
         <xsl:for-each-group select="$groups/xhtml:grp" group-by="xhtml:sort">
            <xsl:choose>
               <xsl:when
                  test="not($base = current-group()/@n) and current-group()//xhtml:sort[normalize-space(.)]">
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <span class="surface">
               ...
               <xsl:choose>
                  <xsl:when
                     test="$h = 'h2' and not(descendant-or-self::tei:addSpan[@type = 'add'] | descendant-or-self::tei:delSpan | descendant-or-self::tei:anchor[@type = 'add'] | descendant-or-self::tei:anchor[@type = 'del'])">
                     <span class="add">
                        <xsl:apply-templates>
   ...
   <xsl:template match="text()[ancestor::tei:w]">
      ...
      <xsl:variable name="addDel">
         ...
         <xsl:value-of
            select="
               if ($addAnchor[@xml:id = substring-after($addSpan/@spanTo, '#')]) then
                  'add'
               else
                  if (($addAnchor and not(exists($addSpan))) or ($addSpan and not(exists($addAnchor)))) then
                     'add'
                  else
                     if ($delAnchor[@xml:id = substring-after($delSpan/@spanTo, '#')]) then
                        'del'
                     else
                        if (($delAnchor and not(exists($delSpan))) or ($delSpan and not(exists($delAnchor)))) then
                           'del'
                        else
                           ()"
         />
      ...
      <xsl:variable name="dam">
         ...
         <xsl:variable name="damageAnchor"
            select="following-sibling::tei:anchor[1][@type = 'damage']"/>
         <xsl:value-of
            select="
               if ($damageAnchor[@xml:id = substring-after($damageSpan/@spanTo, '#')]) then
                  'damage'
               else
                  if (($damageAnchor and not(exists($damageSpan))) or ($damageSpan and not(exists($damageAnchor)))) then
                     'damage'
                  else
                     ()"
         />
