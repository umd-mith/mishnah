...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="files" select="collection(iri-to-uri(concat($path, '?select=[PS][0-9]{5}.xml;recurse=no')))"/>
...
<xsl:copy>
                    ...
                    <xsl:choose>
                        <xsl:when test="current-grouping-key()">
                            ...
                            <xsl:choose>
                                ...
                                <xsl:when
                                    test="not(normalize-space($str)) and current-group()[self::element()]"><xsl:copy-of select="current-group()"/></xsl:when>
                                <xsl:otherwise>
    ...
    <xsl:template
        match="
            tei:milestone | tei:pc | tei:label | tei:surplus |
            tei:fw | tei:gap[not(@reason = 'damage')] | tei:note[ancestor::tei:body]"/>
    ...
    <xsl:template match="tei:num | tei:seg | tei:persName[ancestor::tei:body]">
        <xsl:apply-templates/>
    </xsl:template>
    ...
    <xsl:template
        match="tei:listTranspose | tei:transpose | tei:ptr | tei:supplied | tei:damageSpan | tei:anchor">
    ...
    <xsl:template match="text()[ancestor-or-self::tei:ab]" >
