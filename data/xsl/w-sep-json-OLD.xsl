...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        </witnesses></xsl:variable>
        <xsl:apply-templates select="$xml-for-json" mode="build-json"/>
    </xsl:template>
    <xsl:template
        match="tei:lb | tei:cb | tei:pb | tei:note | tei:label | tei:pc | tei:surplus | tei:fw"/>
        ...
        <xsl:variable name="reg">
            <xsl:variable name="temp">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:value-of
                            select="normalize-space(replace(normalize-unicode($testToNext, 'NFKD'), '[&#x591;-&#x5c7;&#x5f0;-&#x5f4;]', ''))"
                        />
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
    ...
    <xsl:template match="tei:w" mode="build-json">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    ...
                    <xsl:when test="contains(tei:reg,' ')">
                        ...
                        <xsl:for-each select="tokenize(normalize-space(tei:reg), ' ')">
                            ...
                            <xsl:text>", "n" : "</xsl:text>
                            <xsl:value-of select="."/>
                            <xsl:if test="position()=1"><xsl:text>", "id" : "</xsl:text>
                            <xsl:value-of select="$token/tei:w/@id"/></xsl:if>
                            <xsl:text>" }</xsl:text>
                            <xsl:if test="position()!=last()">,</xsl:if>
                        </xsl:for-each>
                    </xsl:when>
                ...
                <xsl:variable name="following-text"><xsl:value-of select="following-sibling::tei:w/tei:reg"/></xsl:variable>
