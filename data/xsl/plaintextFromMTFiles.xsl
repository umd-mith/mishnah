...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="wit" select="'ref'"/>
    <xsl:param name="inclID" select="'n'"/>
    <xsl:param name="chunk" select="'ch'">
    ...
    <xsl:variable name="path">
        <xsl:choose>
            ...
            <xsl:when test="$corpus = 'm'">
                <xsl:value-of
                    select="concat('../tei/', $dir, '?select=', $wit, '*', $dir, '*.xml;recurse=yes')"
                />
            </xsl:when>
        </xsl:choose>
    </xsl:variable>
    ...
    <xsl:template name="chunk-ch">
        <xsl:for-each select="$files//tei:div3">
            <xsl:variable name="fname" select="concat(if ($corpus = 'm') then 'm-' else 't-',@xml:id)"></xsl:variable>
            <xsl:result-document href="file:/C:/users/hlapin/desktop/out/{$fname}.txt" method="text"
                indent="no" encoding="utf-8" omit-xml-declaration="yes" byte-order-mark="yes">
                <xsl:apply-templates select="*"/>
            </xsl:result-document>
