...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <div class="selectionList">
            ...
            <form action="compare" method="get">
                <div class="formDescr">
                    <h3>Select Witnesses<a class="tooltip">[?]<span class="help"><em>Select
                                    Witnesses</em>Use numerals, starting with 1, in the input fields
                                to select witnesses. This will determine the order of output. In
                                "Text/Apparatus" mode the text labeled "1" will be the base
                                text.</span></a></h3>
                    <!-- following is a fudge to get parameters into form output. 
                                Has got to be a less devious way of doing this. -->
                    <input type="text" name="mcite" value="{$mcite}" style="display:none;"/>
                    <input type="text" name="unit" value="{$unit}" style="display:none;"/>
                    <input type="text" name="tractName" value="{$tractName}" style="display:none;"/>
                </div>
    ...
    <xsl:template name="buttons">
        <xsl:choose>
            <xsl:when test="$unit = 'all' or $unit = ''">
                <div class="submit">
                    <input class="submit" type="submit" value="Collate" disabled="disabled"/>
                    <!--<br> </br>
                ...
                <div class="radio">
                    <span>
                        <input id="align" class="radio" type="radio" disabled="disabled"
                            name="alignType"/>
                        <label for="align">Alignment</label>
                    </span>
                    <span>
                        <input id="apparatus" class="radio" type="radio" disabled="disabled"
                            name="alignType" value="apparatus"/>
                        <label for="apparatus">Text/Apparatus</label>
                    </span>
                    <span>
                        <input id="synopsis" class="radio" type="radio" disabled="disabled"
                            name="alignType" value="synopsis"/>
                        <label for="synopsis">Synopsis</label>
                    </span>
                    <h3 class="buttonsHead">Compare<a class="tooltip">[?]<span class="help"
                                    ><em>Compare Texts</em>Select alignment type: Alignment: a
                                word-by-word alignment ("partitur"); Text/Apparatus: text and
                                apparatus (text designated 1 is the base text); Synopsis: selected
                                texts presented in parallel-columns.</span></a></h3>
                </div>
            </xsl:when>
            <xsl:otherwise>
                <div class="submit">
                ...
                <div class="radio">
                    <span>
                        <input id="align" class="radio" type="radio" value="align" name="alignType">
                    ...
                    <span>
                        <input id="apparatus" class="radio" type="radio" name="alignType"
                            value="apparatus">
                    ...
                    <span>
                        <input id="synopsis" class="radio" type="radio" name="alignType"
                            value="synopsis">
                    ...
                    <h3 class="buttonsHead">Compare<a class="tooltip">[?]<span class="help"
                                    ><em>Compare Texts</em>Select alignment type: Word-by-word
                                alignment ("partitur"); text and apparatus (text designated 1 is the
                                base text); parallel-column synopsis.</span></a></h3>
                </div>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="tei:witness[@corresp]" mode="selected">
        ...
        <xsl:if test="some $p in $witsWithText//my:wit-compos/text() satisfies $p = $checkMe">
            <tr>
                <td class="selField">
    ...
    <xsl:template match="tei:witness[@corresp]" mode="noSelected">
        <tr>
            <td class="selField">
                <input type="text" value="" size="3" maxlength="3" disabled="disabled"/>
            </td>
            <td class="siglum">
                <xsl:value-of select="@xml:id"/>
            </td>
    ...
    <xsl:template match="my:m-active-compos | my:ch-compos | my:ord-compos"
        mode="selected noSelected"/>
</xsl:stylesheet>
