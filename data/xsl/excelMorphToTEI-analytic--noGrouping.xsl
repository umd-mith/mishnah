        ...
        <TEI>
            ...
            <text>
                <body>
                    ...
                    <xsl:copy-of select="$anal-segs"></xsl:copy-of>
                    
                            <div
                            corresp="{concat('r:',replace(current-group()[1]/@xml:id,'m-(ref\.[1-6]).*','$1'))}">
                            
                                                <ab
                                                  corresp="{concat('r:',replace(current-group()[1]/@xml:id,'m-(ref\.\d\.\d*?\.\d*?\.\d*?)\..*','$1'))}">

                                                  <xsl:for-each-group select="current-group()"
                                                  group-adjacent="@x-w">
                                                  <w
                                                  corresp="{concat('r:',replace(current-group()[1]/@xml:id,'m-(ref\.\d\.\d*?\.\d*?\.\d*?\.\d*?)\..*','$1'))}">
                                                  <xsl:attribute name="lemma"
                                                  select="current-group()[1]/*:span/text()"/>
                                                  <choice>
    ...
    <xsl:template name="segments">
        ...
        <xsl:for-each select="$cells/*[matches(name(),'pref\d-id')]">
            ...
            <span type="pref">
                ...
                <xsl:attribute name="n"
                    select="substring-before(substring-after(name(),'pref'),'-')"/>
                ...
                <xsl:attribute name="ana">
                    <xsl:call-template name="ana-links">
                        ...
                        <xsl:with-param name="cells"
                            select="following-sibling::*[matches(name(),'pref\d-function')][1]"
                            as="element()"/>
                    </xsl:call-template>
                </xsl:attribute>
        ...
        <xsl:for-each select="$cells/*:lexiconItem">
            ...
            <span type="base">
                ...
                <xsl:value-of select="if ($cells/*:middle) then $cells/*:middle else . "/>
                <span type="lemma">
        ...
        <xsl:for-each select="$cells/*:suffix-surface-including-infix">
            ...
            <span type="suff">
                ...
                <xsl:attribute name="ana">
                    <xsl:call-template name="ana-links">
                        <xsl:with-param name="cells"
                            select="for $i in $cells/*[contains(name(),'suff')] return $i"/>
        ...
        <xsl:choose>
            ...
            <xsl:when test="$mType = 'base'">
                ...
                <xsl:if test="$cells[name() = 'binyan']">
                    <xsl:value-of
                        select="translate(lower-case(concat(' m:',$cells[name() = 'binyan'])),'&apos;&apos;','')"
                    />
                </xsl:if>
                ...
                <xsl:choose>
                    ...
                    <xsl:when
                        test="$cells[name() = 'definiteness'] ='singular' or $cells[name() = 'definiteness'] ='plural'">
            ...
            <xsl:when test="$mType = 'suff'">
                ...
                <xsl:choose>
                    <xsl:when test="$cells[name() = 'suff-function']">
                        <xsl:choose>
                            <xsl:when
                                test="$cells[name() = 'suff-function'] = 'accusative or nominative'">
                    ...
                    <xsl:when test="$cells[name() = 'suff-person']">
                        ...
                        <xsl:choose>
                            <xsl:when
                                ...
                                <xsl:value-of
                                    select="concat(' m:p_',$cells[name() = 'suff-person'])"/>
                            </xsl:when>
                ...
                <xsl:choose>
                    <xsl:when test="$cells[name() = 'suff-gender']">
                        <xsl:choose>
                            ...
                            <xsl:when
                                test="$cells[name() = 'suff-gender'] = 'masculine and feminine'">
                                <xsl:text> m:MorF</xsl:text>
                            </xsl:when>
                        </xsl:choose>
                    </xsl:when>
