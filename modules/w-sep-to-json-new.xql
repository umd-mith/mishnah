...
(:  get tokenized text  :)
declare function local:nodes ($mCite as xs:string, $witNames as xs:string*) as element()+ {
   ...
   return
      let $doc := doc(concat($config:data-root, 'mishnah/w-sep/', $witName, '-w-sep.xml')) return
         if ($doc/id(concat($witName,'.',$mCite))) then
            ...
            return
            local:copy($extract/*/parent::*) (: why is this axis necessary? :)
         else ()
};
...
declare function local:processWTokens($ab as element()+) as node()+ {
   let $add := local:filter-w-set($ab, 'add')
   let $del := local:filter-w-set($ab, 'del')
   ...
   return
      ...
      return
         ...
         let $groupedAddDel := local:recurseAddDel($firstH1H2, $addDel)
            ...
            if ($el/name() = 'h1h2') then
               ...
               let $h2 := 
                  ...
                  return
                     ...
                     (:else
                        ():)
               let $processedH2 := local:h1h2($h2,'h2')
               return 
...
declare function local:recurseAddDel($w as element()?, $addDel as element()*) as element()* {
   let $next := $w/following-sibling::*[1]
   return
      if ($w) then
         if ($w = $addDel) then
            (<h1h2>{$w, local:groupAddDel($w, $addDel)}</h1h2>,
            local:recurseAddDel($w/following-sibling::*[not(. = $addDel)][1], $addDel))
         else
            ($w, local:recurseAddDel($next, $addDel))
      else
         ()
};

(: Adds to group as necessary  :)
declare function local:groupAddDel($w as element(), $addDel as element()*) as element()* {
   let $next := $w/following-sibling::*[1]
      where $next[. = $addDel]
   return
      ($next, local:groupAddDel($next, $addDel))
};
   ...
   "witnesses" : for $ab in $wSequence return 
     ...
     (:let $witnessTokens := 
         if ($addIDs[@xml:id = $ab/@xml:id][node()] | $delIDs[@xml:id = $ab/@xml:id][node()]) then
            ...
            $ab:)
     let $wGroups := local:wordGroups($ab//w)
         return
...
declare function local:getTokenData($mcite as xs:string, $wits as xs:string*) {
   (: get only ws and string that are not between comm span and anchor:)
   (: instead do this after? :)
   (: get nodes :)
   let $nodes := local:nodes($mcite, $wits)
   let $noComm := for $ab in $nodes
   return
      <ab
         xml:id="{$ab/@xml:id}">{
            $ab/* except local:filter-w-set($ab, 'comm')
         }</ab>
   ...
   let $listOfTokens :=
      ...
      return
         local:processWTokens($srcTokens)
   return
      local:buildJSON($listOfTokens)

};

local:getTokenData($mCite, $witNames)