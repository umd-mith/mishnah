...
declare function local:do-single-or-group($in as xs:string, $type as xs:string) as element()*{
   ...
   if (string-length(translate($in,'_ ','')) < string-length($in)) then
      element {$name} {
            for $t in tokenize($in, if ($type = 'tkn') then '_' else ' ')
            return
               element {$type} {
...
declare function local:toJSON ($seq as element()+) as item()* {
   for $e in $seq return
   typeswitch ($e)
      case element(tknGrp) return  if ($e/normalize-space()) then map{name($e) : local:toJSON($e/*)} else ()
      ...
      case element(tkn) return if ($e/normalize-space()) then map{name($e) : array{local:toJSON($e/*)}} else ()
...
declare function morph:pseudoMorph($tkns as xs:string, $expans as xs:string, $type as xs:string){
   let $out:=
   (local:do-single-or-group($tkns,'tkn'),
