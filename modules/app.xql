...
declare function app:index-compos($unit as xs:string, $mcite as xs:string) {
  ...
  <parameters>
    <param
      name="tei-loc"
      value="{$config:data-root}/mishnah/"/>
  </parameters>)
  return
    $out
};
...
declare function app:page_assets($node as node(), $model as map(*)) {
  if ($model("resource") = "align") then
    ...
    if ($model("resource") = "edit") then
      (<link
        xmlns="http://www.w3.org/1999/xhtml"
        href="$app-root/resources/editapp/ngDialog.css"
        rel="Stylesheet"
        type="text/css"/>,
      <link
        xmlns="http://www.w3.org/1999/xhtml"
        href="$app-root/resources/editapp/ngDialog-theme-default.css"
        rel="Stylesheet"
        type="text/css"/>,
      <link
        xmlns="http://www.w3.org/1999/xhtml"
        href="$app-root/resources/editapp/angMishnah.css"
        rel="Stylesheet"
        type="text/css"/>)
    else
      if ($model("resource") = "read") then
        (<link
          xmlns="http://www.w3.org/1999/xhtml"
          href="$app-root/resources/css/FormattingforHTML.css"
          rel="Stylesheet"
          type="text/css"/>,
        <link
          xmlns="http://www.w3.org/1999/xhtml"
          href="$app-root/resources/css/demo-styles.css"
          rel="Stylesheet"
          type="text/css"/>)
      else
        if ($model("resource") = "compare") then
          (<link
            xmlns="http://www.w3.org/1999/xhtml"
            href="$app-root/resources/css/compare-home.css"
            rel="Stylesheet"
            type="text/css"/>,
          <script
...
declare function app:expand-mcite($mcite as xs:string) {
  ...
  then
    (concat($mcite_parts[last() - 1], '.', $mcite_parts[last()]))
  else
    $mcite_parts[last()]
  return
    concat(
        ...
        return
          <tr>
            <td
              ...
              <a
                href="browse/{data($w/@xml:id)}/page/{
                    substring-after(($source//tei:body//tei:pb)[1]/@xml:id, concat($w/@xml:id, '.'))
                  }">{data($w/@xml:id)}</a></td>
            <td>{
                $w/text()
              }
              (<span
                style="font-size:75%;">
                <a
                  href="{concat($config:http-data-root, "/mishnah/", substring-before($w/@corresp, '.xml'))}.xml">TEI/XML</a>
              </span>)
            </td>
          </tr>
        )
...
declare function app:read($node as node(), $model as map(*)) {
  element div {
    ...
    if ($mode = 'chapter')
    then
      transform:transform($step1, doc("//exist/apps/digitalmishnah/xsl/flat-to-pages.xsl"), ())
    else
      $step1
    ...
    <parameters>
      ...
      <param
        name="tei-loc"
        value="{$config:http-data-root}/mishnah/"/>
    </parameters>)
    return
      $step3
  }
  ...
  return
    element ul {
      ...
      return
        ...
        return
          <li
            class="list-group-item wit-item"
            id="wit-item-{data($w/@xml:id)}"
            draggable="false">
            <input
              type="checkbox"
              class="wit-remove"/>&#160;<strong>{data($w/@xml:id)}</strong>
            <!--<span>&#160;{$w/text()}</span>               
                              ...
                              return
                                ...
                                then
                                  ...
                                  return
                                    ...
                                    <li
                                      ...
                                      <div
                                        ...
                                        role="tabpanel"
                                        id="{$htmlid}"
                                        aria-labelledby="{$htmlid}_Heading"
                                        aria-expanded="false"
                                        style="height: 0px;">
                                        <ul
                                          class="list-group">{
                                  ...
                                  <li
                                    ...
                                    <a
                                      class="mishnah_link mishnah_chap"
                                      href="#{$chap/@n}">
                                      Chapter {substring-after($chap/@n, concat($chap/parent::my:tract-compos/@n, '.'))}
                                    </a>
                                  </li>
